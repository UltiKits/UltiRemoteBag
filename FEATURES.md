...
| ultiremotebag.config.remotebag.save_on_close | Whether closing the content GUI in edit mode writes the page. When true (the shipped default, and what every server did unconditionally before `UltiKits/UltiRemoteBag#18` wired this key), `RemoteBagContentGUI#onClose` copies the live inventory into the cache and persists it. When false, closing discards whatever was not saved by the page's own Save button or `/bag save`. The key governs the save and nothing else: the page's lock is released either way, so a page closed with this off does not stay locked until `lock.timeout_seconds` | config | `config/remotebag.yml: save_on_close (default: true)` | n/a | n/a | admin | brief | RemoteBagContentGUI#onClose |
