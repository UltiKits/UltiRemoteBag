...
| ultiremotebag.bag.save | Manually persist the sender's bag pages to the database. Flushes the sender's own currently open content page first when that page is in edit mode (`RemoteBagContentGUI#flushOpenEditPage`, which resolves the open page through the GUI library's own player-to-page map), so an item placed since the page was opened is persisted without closing it or clicking its Save button. A read-only page is deliberately not flushed — its live inventory is another player's bag being viewed — and the lookup is keyed by the sender, so the command can never write another player's open page. Before `UltiKits/UltiRemoteBag#22` this command persisted only what had already reached the service cache | command | `/bag save` | ultibag.use | player | player | detailed | BagCommand#saveBag, RemoteBagContentGUI#flushOpenEditPage |
