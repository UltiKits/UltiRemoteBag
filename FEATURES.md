...
| ultiremotebag.bag.cleanup-on-quit | On player quit, release every bag-page lock the quitting player held (owner or admin) — the one independently observable effect — and call `saveBag`/`clearCache` on their cached pages; per `UltiKits/UltiRemoteBag#23` the save call is redundant in practice (the cache is never left dirty by any reachable code path), so only the lock release is actually testable through this event | event | quit the server as any player who has opened at least one remote bag this session | n/a | n/a | internal | brief | BagListener#onPlayerQuit |
| ultiremotebag.bag.auto-save | Declared to persist every currently cached (loaded) player's bag pages to the database every 300 seconds (hardcoded; `auto_save_interval` has no effect, `UltiKits/UltiRemoteBag#13`). In practice this task has NO observable effect: every code path that writes to the cache (`RemoteBagContentGUI#saveCurrentContents`, `RemoteBagService#createBagPage`, `#clearBagPage`) immediately calls `saveBag()` synchronously in the same method, so the cache is never left dirty for this scheduled task to catch. Known product defect, `UltiKits/UltiRemoteBag#23` | scheduled | runs automatically every 6000 ticks (300s, hardcoded), but reaches no dirty entry to persist — see Source for why | n/a | n/a | internal | detailed | RemoteBagService#autoSaveTask |
