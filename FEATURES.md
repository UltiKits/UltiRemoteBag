...
- **Source:** `ClassName#member` — the class and member that actually reads or applies the
  feature — for every Kind, `config` included: all 18 `config` rows below cite the reading
...
`@ConditionalOnConfig` = 0, `@ConfigEntity` = 1 (class), `@ConfigEntry` = 18 — confirmed by
...
| ultiremotebag.lifecycle.removed-key-warning | At module load, log one `WARNING` per setting that 6.3.0 removed and that is still present in the operator's `config/remotebag.yml`, naming the module, the file and the key, and saying where that setting's job went instead. The six are `auto_save_interval`, `gui_title`, `messages.no_permission`, `messages.page_locked`, `messages.bag_saved` and `save_on_close` (`UltiKits/UltiRemoteBag#13`, `#14`, `#15`, `#16`, `#17`, `#18`, `#23`). It exists because removing a key from the code does not remove it from anybody's file: the framework writes a missing key's default in on first boot and never deletes a key it no longer declares, so an operator who had edited one of the six would otherwise see no trace of the removal at all. A fresh install has none of the six and logs nothing; a key this module still reads is not reported, so a warning means a residual key and not merely boot | event | start the server (or `/upm install`) with a `config/remotebag.yml` that still holds one of the six keys | n/a | n/a | admin | brief | UltiRemoteBag#registerSelf, RemovedConfigKeys#warnIfStillPresent |
...
`RemoteBagConfig` (`config/remotebag.yml`, 18 keys total — matching the reconciliation table's own
`@ConfigEntry` count of 18 exactly).

**Every key below is read by production code.** Until 6.3.0 seven were not. Six were removed
...
`#18`, `#23`) and one was wired to the behaviour it declares (`lock.notify_readonly_viewers` —
`UltiKits/UltiRemoteBag#19`). `save_on_close` was wired first and the maintainer reversed that on
2026-09-23: the branch it switched off had no way to return the window's contents, so turning it off
destroyed them. Wiring a dead key only removes the defect when the branch the key switches off is
itself safe (feature request `UltiKits/UltiRemoteBag#37`). Confirmed for each remaining key by
...
longer declares — so all of them are still on disk on every server that has run this module. See
...
| ultiremotebag.config.remotebag.rows_per_page | How much one bag page holds, in rows of 9 slots (1-6): the floor for the array `RemoteBagService#deserializeItems` allocates on load, the size `RemoteBagService` allocates for a new or cleared page, and the denominator of the `Slots Used: x/y` lore on the main GUI's page icon. Storage capacity only — it decides how much fits on a page, not how large the window is. `RemoteBagContentGUI` shows and saves a fixed 45-slot (5-row) content area whatever this key holds, and that is deliberate, not an omission: the maintainer's 2026-09-22 decision on `UltiKits/UltiRemoteBag#24` kept the behaviour and the name and corrected the declaration instead, because the behaviour is right and only the promise was too wide, and a rename would orphan every operator's current value. What it also no longer does is lose items: `deserializeItems` sizes the page to `max(rows_per_page * 9, highest stored slot + 1)`, bounded by the 54 slots this key's own `@Range(min = 1, max = 6)` makes addressable, so an item saved from one of the GUI's still-exposed higher slots is read back rather than triggering the `ArrayIndexOutOfBoundsException` that was previously caught and turned into an empty page. A single unreadable slot key — non-numeric, negative, non-canonical (`+5`, `05`, whitespace-padded: these parse to the same index as their plain form and used to collide with it, dropping one item with no warning at all), or beyond those 54 slots — is skipped with a `WARNING` naming the page and the key rather than discarding the page, and that warning now carries the module's own `[UltiTools] [UltiRemoteBag]` prefix like every other line this module emits. The ceiling is applied before the array is allocated, so no stored key can size the allocation, and it is computed from `RemoteBagConfig.MAX_ROWS_PER_PAGE * SLOTS_PER_ROW` rather than restated as a literal, so widening this key's range cannot leave the loader's ceiling behind | config | `config/remotebag.yml: rows_per_page (default: 6; storage capacity only — the 45-slot content area does not follow it, by decision, see UltiKits/UltiRemoteBag#24)` | n/a | n/a | admin | detailed | RemoteBagMainGUI#createBagIcon, RemoteBagService#deserializeItems |
