...
| ultiremotebag.bag.cleanup-on-quit | `language: en`; the player has run `ultiremotebag.bag.open-page` above (holding the OWNER lock on that page) and placed an unsaved item into the open GUI, without running `/bag save` and without closing the GUI | Quit the server (disconnect, not kick) while still holding the lock and the unsaved change | `BagListener#onPlayerQuit` released every lock the quitting player held — confirmed by a second, OP'd admin running `ultiremotebag.admin.see` above against the same page afterward and receiving EDIT mode (not read-only/blocked) immediately, with no stale lock; the unsaved item is present in `remote_bags` even though `/bag save` was never run manually, and the quitting player's bag cache entry was cleared (`RemoteBagService#clearCache`) | server | |
...
| ultiremotebag.admin.see.neg-no-bags | `language: en`; admin holds `ultibag.use` AND `ultibag.admin.see`; target has played but currently owns zero bag pages (every page deleted first via `ultiremotebag.admin.delete`) | Run `/bag see <target>` | Chat line reads `Player <target> has no bags` (yellow, `player_no_bags`); no GUI opens | server | |
